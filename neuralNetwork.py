...
class NeuralNetworkClassifier():
    ...
    def calculate_loss(self, X, y):
        ...
        hidden_activations = sigmoid(X.dot(self.inputWeights.T[:-1, :]) + self.inputWeights.T[-1, :])
        output_activations = sigmoid(hidden_activations.dot(self.outputWeights.T[:-1, :]) + self.outputWeights.T[-1, :])

        # Calculate cross-entropy loss
        if self.output > 1:
            # One-hot encoded labels for multi-class classification
            cross_entropy_loss = -np.sum(y * np.log(output_activations), axis=1)
        else:
            # Binary labels for binary classification
            cross_entropy_loss = -(y * np.log(output_activations) + (1 - y) * np.log(1 - output_activations))

        # Calculate L2 regularization term
        l2_reg = (np.sum(self.outputWeights[:, :-1] ** 2) + np.sum(self.inputWeights[:, :-1] ** 2)) * self.l / 2

        # Calculate the total loss
        loss = cross_entropy_loss.mean() + l2_reg
